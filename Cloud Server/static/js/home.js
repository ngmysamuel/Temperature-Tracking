window.onload = function(){
	...
	var elementEvent = document.getElementById("eventTable");
...
async function deactivateLocalLockdown(localStatus) {
	let e = document.getElementById("radioGroup").value
	const response = await fetch('http://localhost:5000/api/deactivatelocallockdown/'+e);
	getLocalStatus(localStatus)
}
...
async function getLocalStatus(localStatus) {
	const response = await fetch('http://localhost:5000/api/getAllLocalStatus');
	const data = await response.json(); //extract JSON from the http response
	for (var key of Object.keys(data)) {
		console.log("District: " + key + " is under lockdown: " + data[key])
	}
	console.log(data)
	localStatus.innerHTML = JSON.stringify(data);
}
